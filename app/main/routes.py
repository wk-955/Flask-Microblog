...
def explore():
    ...
    return render_template('index.html', title='搜索', posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def follow(username):
    ...
    db.session.commit()
    flash(_('你已经关注 %(username)s!', username=username))
    return redirect(url_for('main.user', username=username))
...
def unfollow(username):
    ...
    db.session.commit()
    flash(_('你已不再关注 %(username)s!', username=username))
    return redirect(url_for('main.user', username=username))
...
def translate_text():
    q = request.form['text']
    fromLang = request.form['source_language']
    toLang = request.form['dest_language']
    return jsonify({'text': translate(q, fromLang, toLang)})
    # q = request.form['q']
...
def search():
    ...
    prev_url = url_for('main.search', q=g.search.form.q.data, page=page-1) if page>1 else None
    return render_template('search.html', title=_('搜索'), posts=posts, next_url=next_url, prev_url=prev_url)
...
def send_message(recipient):
    ...
    return render_template('send_message.html', title=_('发信息'), form=form, recipient=recipient)
